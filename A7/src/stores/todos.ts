...
/**
 * Generates a fresh ISO timestamp for create and update payloads that require sync metadata.
 */
function nowIso() {
...
/**
 * Centralizes Todo entity collections and CRUD actions for the dashboard.
 */
export const useTodoStore = defineStore("todos", () => {
  /**
   * Holds all loaded categories from the backend.
   */
  const categories = ref<TodoCategory[]>([]);
  /**
   * Holds all loaded priorities from the backend.
   */
  const priorities = ref<TodoPriority[]>([]);
  /**
   * Holds all loaded tasks from the backend.
   */
  const tasks = ref<TodoTask[]>([]);
  /**
   * Tracks whether Todo data is currently loading.
   */
  const isLoading = ref(false);
  /**
   * Stores the latest Todo-related error message for the UI.
   */
  const error = ref("");

  /**
   * Returns only tasks that are not archived so the main work list stays focused.
   */
  const activeTasks = computed(() => tasks.value.filter((task) => !task.isArchived));
  /**
   * Returns archived tasks separately for restore/delete actions.
   */
  ...
  async function loadAll() {
    ...
    error.value = "";
    try {
      // Load all Todo entity groups together so the dashboard can render one consistent snapshot.
      ...
      // Stores keep the formatted error text so views can stay mostly declarative.
  ...
  async function addCategory(input: { categoryName: string; categorySort: number; tag?: string }) {
    const created = await todoApi.createCategory({
      // Trim text before sending it so UI whitespace does not leak into backend data.
      categoryName: input.categoryName.trim(),
      categorySort: input.categorySort,
      tag: input.tag?.trim() || null,
    });

    // Re-sort locally so the UI reflects categorySort immediately after creation.
  ...
  async function saveCategory(category: TodoCategory) {
    ...
    // Replace only the edited entity instead of reloading the entire collection.
  ...
  async function removeCategory(id: string) {
    ...
    // Remove orphaned tasks locally because they can no longer point to a deleted category.
  ...
  async function addPriority(input: { priorityName: string; prioritySort: number }) {
    const created = await todoApi.createPriority({
      ...
      // The current backend contract still expects a sync timestamp on priority creation.
      syncDt: nowIso(),
    });
  ...
  async function savePriority(priority: TodoPriority) {
    const updated = await todoApi.updatePriority(priority.id, {
      ...
      // Send back the server-tracked sync timestamp expected by the update contract.
  ...
  async function removePriority(id: string) {
    ...
    // Remove orphaned tasks locally because they can no longer point to a deleted priority.
  ...
  async function addTask(input: {
    ...
    const created = await todoApi.createTask({
      ...
      // Convert browser datetime-local values into ISO strings before sending them to the API.
    ...
    // Re-sort locally so manual task ordering is reflected without another fetch.
  ...
  async function saveTask(task: TodoTask) {
    const updated = await todoApi.updateTask(task.id, {
      ...task,
      // Normalize edited due dates back into API format before saving.

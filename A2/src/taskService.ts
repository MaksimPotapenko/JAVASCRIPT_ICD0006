...
async function validateCategoryAndPriority(categoryId: string | undefined, priority: Priority): Promise<void> {
...
export interface AddTaskInput {
  ...
  categoryId?: string;
  dependencies?: string[];
  recurrence?: string; // parsed from text input
}
...
export async function addTask(input: AddTaskInput): Promise<Task> {
  ...
  const dependencies = Array.isArray(input.dependencies)
    ? input.dependencies.filter((x): x is string => typeof x === 'string').map(x => x.trim()).filter(Boolean)
    : [];
...
export async function listTasks(filters?: TaskFilters, sort?: { field: SortField; order: SortOrder }): Promise<Task[]> {
  const tasks = await loadTasks();
  ...
  if (filters) {
    out = out.filter(t => {
      ...
      if (filters.overdue === true && !isOverdue(t, today)) return false;
      return true;
    });
  }
    ...
    out = sortBy(out, t => {
      switch (sort.field) {
        ...
        case 'updatedAt':
          return t.updatedAt;
        default: {
          const exhaustive: never = sort.field;
          return exhaustive;
        }
      }
    }, sort.order);
...
export async function updateTask(id: string, patch: UpdateTaskInput): Promise<Task> {
  ...
  if (patch.priority !== undefined) {
    ...
    await validateCategoryAndPriority(
      patch.categoryId === null ? undefined : (patch.categoryId ?? task.categoryId),
      patch.priority
    );
  ...
  if (patch.dependencies !== undefined) {
    ...
    if (task.dependencies.includes(task.id)) throw new Error('Task cannot depend on itself');

    const ids = new Set(tasks.map(t => t.id));
    const missing = task.dependencies.filter(d => !ids.has(d));
    if (missing.length > 0) throw new Error(`Dependency not found: ${missing.join(', ')}`);
  }
  ...
  if (patch.status !== undefined) {
    ...
    if (patch.status === 'done' && task.dependencies.length > 0) {
      ...
      if (incomplete.length > 0) {
        throw new Error(
          `Cannot complete task. Incomplete dependencies: ${incomplete.map(t => t.id).join(', ')}`
        );
      }
    }
    ...
    if (!wasDone && patch.status === 'done' && task.recurrence) {
      ...
      if (!end || compareIsoDates(nextDue, end) <= 0) {
        ...
        const alreadyExists = tasks.some(
          t => (t.seriesId ?? t.id) === seriesId && t.dueDate === nextDue
        );
...
export async function getStatistics(): Promise<TaskStatistics> {
  const tasks = await loadTasks();
  const today = todayIso();
  ...
  for (const t of tasks) {
    (byStatus as Record<TaskStatus, number>)[t.status] += 1;
    (byPriority as Record<Priority, number>)[t.priority] += 1;
  }
  ...
  const done = (byStatus as Record<TaskStatus, number>)['done'];
...
export async function addCategory(input: AddCategoryInput): Promise<Category> {
  ...
  for (const p of allowed) assertPriority(p);
...
export async function updateCategory(id: string, patch: Partial<Pick<Category, 'name' | 'allowedPriorities'>>): Promise<Category> {
  assertNonEmptyString(id, 'id', 1);
  const cats = await loadCategories();
  ...
  if (patch.allowedPriorities !== undefined) {
    for (const p of patch.allowedPriorities) assertPriority(p);
...
export async function categoryStats(): Promise<Record<string, { total: number; byPriority: Record<Priority, number> }>> {
  const [tasks, cats] = await Promise.all([loadTasks(), loadCategories()]);
  const byCat = groupBy(tasks.filter(t => t.categoryId), t => t.categoryId as string);

  const out: Record<string, { total: number; byPriority: Record<Priority, number> }> = {};
  for (const cat of cats) {
    const tasksIn = byCat[cat.id] ?? [];
    const byP: Record<Priority, number> = { low: 0, medium: 0, high: 0 };
    for (const t of tasksIn) byP[t.priority] += 1;

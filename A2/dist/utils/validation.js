...
export function assertNonEmptyString(value, field, minLen = 1) {
...
export function assertPriority(value) {
    if (value !== 'low' && value !== 'medium' && value !== 'high') {
        throw new Error('Invalid priority');
    }
}
export function assertIsoDateOrEmpty(value, field) {
...
export function parseRecurrence(raw) {
    ...
    const rule = { frequency: freq, interval: Math.floor(interval) };
    for (const part of rest) {
        const [k, v] = part.split('=').map(x => x.trim());
        if (k === 'end' && v) {
            if (!isIsoDate(v))
                throw new Error('Recurrence end must be YYYY-MM-DD');
            rule.endDate = v;
        }
    }
    return rule;
